...
impl Server {
    ...
    /// Whether the connection is still negotiating its TLS handshake.
    fn is_handshaking(conn: &Connection) -> bool {
        let Some(shared) = conn.metadata.stream.0.as_ref() else {
            return false;
        };
    ...
    /// closes idle and still-handshaking connections immediately and keeps
    /// servicing only connections with a pending response until they are
    /// flushed or `drain_timeout` elapses.
    fn event_loop(
        ...
        loop {
            ...
            if !draining && shutdown.is_triggered() {
                ...
                // Idle keep-alive connections have nothing in flight and
                // handshakes never produced a request; close them right away
                // and keep only responses waiting to flush.
                connections
                    .retain(|_, conn| !conn.write_buf.is_empty() && !Self::is_handshaking(conn));
            ...
            for (i, item) in poll_fds.iter_mut().enumerate() {
                ...
                if let Some(conn) = connections.get_mut(&fd) {
                    let handshaking = Self::is_handshaking(conn);
