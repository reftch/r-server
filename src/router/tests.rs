...
mod tests {
    ...
    fn response<'a>(metadata: &'a ConnectionMetadata<TestStream>) -> Response<'a> {
        Response::new(metadata, Status::Ok, "", ContentType::TEXT)
    }
    ...
    fn test_route_with_query_params() {
        ...
        let mut req = Request::parse(buf).expect("Should parse");
    ...
    fn test_different_methods() {
        ...
        let mut res_get = response(&metadata);
        ...
        let mut res_post = response(&metadata);
